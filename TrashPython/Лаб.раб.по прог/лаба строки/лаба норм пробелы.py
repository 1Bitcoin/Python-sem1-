...
for item in range(len(matr)):
    ...
    for letter in range(len(matr[item])):
        ...
        if mar[letter].isdigit() and not mar[letter-1].isdigit() and not mar[letter-1] == '+' and not mar[letter-1] == '-':
            ...
            copp = ""
            i = 1
for sc in range(len(matr)):
    for hhj in range(len(Old)):
        matr[sc] = matr[sc].replace(str(Old[hhj]), str(New[hhj]))
    print(matr[sc])
''' wordforreplace = copp
    newword = ''
    word = ""
    wholeitem = ""
    mart = matr[item]
    for letter1 in range(len(mart[item])):          
            if (mart[letter1].isdigit() or mart[letter1] == '+' or mart[letter1] == '-') and (not mart[letter1-1] == '-' or not mart[letter1] == '+'):
                word += letter1
            else:              
                if word == wordforreplace:
                    word = newword
